...
class BLOCKids_Configurator_Plans
{
     ...
     * 
     * VERZE: 2.1.0 - Batch načítání desků ve workspace (1 dotaz místo N)
     */
    private static function format_plan_response($plan)
    {
        ...
        // ===== WORKSPACE: Batch načtení všech desků najednou =====
        ...
        if (is_array($raw_workspace) && !empty($raw_workspace)) {
            ...
            // 1. Sesbírat všechna unikátní desk ID z workspace
            $desk_ids = array();
            foreach ($raw_workspace as $item) {
                if (is_array($item) && isset($item['id']) && $item['id']) {
                    $desk_ids[] = (int) $item['id'];
                }
            }
            $desk_ids = array_unique($desk_ids);

            // 2. Načíst všechny produkty najednou (1 batch místo N dotazů)
            $desk_cache = array();
            foreach ($desk_ids as $did) {
                $p = wc_get_product($did);
                if ($p) {
                    $desk_cache[$did] = array(
                        'id'    => $p->get_id(),
                        'title' => $p->get_name(),
                        'price' => (int) $p->get_price(),
                        'image' => wp_get_attachment_url($p->get_image_id()) ?: '',
                        'type'  => get_post_meta($did, '_blockids_type', true) ?: 'rectangle',
                    );
                }
            }

            // 3. Sestavit workspace z již načtených dat
            $workspace = array();
    ...
    private static function calculate_price($data)
    {
        ...
        if (isset($data['customWidth']) && isset($data['calculatedWidth'])) {
            if (
                (int) $data['customWidth'] < (int) $data['calculatedWidth'] ||
